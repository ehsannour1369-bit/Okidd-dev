...
import { Video, Plus, Trash2, Clock, X, ChevronDown } from "lucide-react";
...
function getColors(scope: "school" | "branch"): Colors {
  if (scope === "school") {
    return {
      accent: "#6366f1", dark: "#4f46e5", light: "#818cf8",
      text: "#1e1b4b", text2: "#3730a3",
      bg: "rgba(255,255,255,0.88)", border: "rgba(99,102,241,0.20)",
      inputBg: "rgba(245,243,255,0.9)",
      dayColors: ["#6366f1","#3b82f6","#8b5cf6","#06b6d4","#10b981","#f59e0b"],
      subtitle: "مدیریت برنامه هفتگی و جلسات کلاس آنلاین",
    };
  }
  return {
    accent: "#0d9488", dark: "#0f766e", light: "#2dd4bf",
    text: "#134e4a", text2: "#0f766e",
    bg: "rgba(255,255,255,0.88)", border: "rgba(13,148,136,0.20)",
    inputBg: "rgba(240,253,250,0.9)",
    dayColors: ["#0d9488","#10b981","#06b6d4","#2dd4bf","#14b8a6","#059669"],
    subtitle: "مدیریت برنامه هفتگی و جلسات آنلاین شعبه",
  };
}
...
export default function OnlineClassManager({ scope }: { scope: "school" | "branch" }) {
  const { user } = useAuthStore();
  const qc = useQueryClient();
  const C = getColors(scope);
  ...
  const IS: React.CSSProperties = {
    width: "100%", background: C.inputBg, border: `1px solid ${C.accent}30`,
  ...
  const cardStyle: React.CSSProperties = {
    ...
    borderRadius: 18, padding: 20, backdropFilter: "blur(12px)",
  };

  const classesParam = scope === "school"
    ? `schoolId=${(user as any)?.schoolId ?? 0}`
    : `branchId=${(user as any)?.branchId ?? 0}`;

  const { data: classes = [] } = useQuery<any[]>({
    queryKey: ["classes-for-online", scope, scope === "school" ? (user as any)?.schoolId : (user as any)?.branchId],
    queryFn: () => api.get(`/classes?${classesParam}`),
    enabled: !!user,
  });
  ...
  const { data: classTeachers = [] } = useQuery<any[]>({
    queryKey: ["class-teachers-for-schedule", scope, selectedClassId],
  ...
  const addMutation = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["class-schedules", selectedClassId] });
      setShowAddForm(false);
      setForm({ dayOfWeek: "0", startTime: "08:00", endTime: "09:30", subject: "", teacherId: "" });
      showToast("ردیف برنامه اضافه شد", "success");
    },
  ...
  for (let d = 0; d < 6; d++) {
    byDay[d] = schedules.filter((s: any) => s.dayOfWeek === d).sort((a: any, b: any) => a.startTime.localeCompare(b.startTime));
  }
  ...
  function Btn({ children, onClick, color = C.accent, small, ghost, disabled }: any) {
    return (
      <button onClick={onClick} disabled={disabled} style={{
        ...
        opacity: disabled ? 0.6 : 1, fontFamily: "Vazirmatn, sans-serif",
    ...
    <div dir="rtl" style={{ fontFamily: "Vazirmatn, sans-serif", paddingBottom: 32 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 24 }}>
        ...
        <div>
          ...
          <p style={{ margin: 0, fontSize: 13, color: C.text2 }}>{C.subtitle}</p>
        </div>
      </div>
        ...
        <>
          ...
          {tab === "schedule" && (
            <>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(150px, 1fr))", gap: 12 }}>
                {DAYS.map((day, d) => (
                  <div key={d} style={{ ...cardStyle, padding: 16 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12, paddingBottom: 10, borderBottom: `2px solid ${C.dayColors[d]}33` }}>
                      <span style={{ width: 10, height: 10, borderRadius: "50%", background: C.dayColors[d], display: "inline-block" }} />
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                      {byDay[d].map((item: any) => (
                        <div key={item.id} style={{ background: `${C.dayColors[d]}10`, border: `1px solid ${C.dayColors[d]}28`, borderRadius: 10, padding: "8px 10px", position: "relative" }}>
                          ...
                          <button onClick={() => deleteMutation.mutate(item.id)} style={{ position: "absolute", top: 6, left: 6, background: "rgba(239,68,68,0.1)", border: "1px solid rgba(239,68,68,0.2)", borderRadius: 7, width: 22, height: 22, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", color: "#ef4444" }}>
                            <Trash2 size={11} />
                          </button>
                        </div>
                      ))}
          ...
          {tab === "sessions" && (
            <>
              ...
              {sessions.length === 0 && (
                <div style={{ ...cardStyle, textAlign: "center", color: "#9ca3af", fontSize: 14, padding: 32 }}>هیچ جلسه‌ای ثبت نشده است</div>
              )}
              <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                {sessions.map((s: any) => (
                  <div key={s.id} style={{ ...cardStyle, display: "flex", alignItems: "center", gap: 16, padding: "14px 20px" }}>
