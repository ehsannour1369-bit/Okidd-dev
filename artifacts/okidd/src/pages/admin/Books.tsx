  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 24 }}>
        <div>
          <h1 style={{ fontSize: 22, fontWeight: 800, color: "#f8f5ff", margin: 0 }}>کتاب‌ها</h1>
          <p style={{ color: "#8b5cf6", fontSize: 14, marginTop: 4 }}>{books.length} کتاب</p>
        </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: 16 }}>
        {books.map(book => (
          <div key={book.id} style={{ background: "rgba(30,18,60,0.85)", border: "1px solid rgba(139,92,246,0.2)", borderRadius: 14, padding: 20, transition: "all 0.3s ease" }}
            onMouseOver={e => { (e.currentTarget as HTMLElement).style.borderColor = "rgba(124,58,237,0.5)"; (e.currentTarget as HTMLElement).style.boxShadow = "0 0 20px rgba(124,58,237,0.2)"; }}
          ...
          >
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
              <h3 style={{ margin: 0, color: "#f8f5ff", fontSize: 15, fontWeight: 700 }}>{book.title}</h3>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 16 }}>
              <div style={{ background: "rgba(124,58,237,0.1)", borderRadius: 8, padding: "8px 12px", textAlign: "center" }}>
                <div style={{ fontSize: 18, fontWeight: 700, color: "#a855f7" }}>{book.lessonCount}</div>
                <div style={{ fontSize: 11, color: "#8b5cf6" }}>درس</div>
              </div>
              <div style={{ background: "rgba(245,158,11,0.1)", borderRadius: 8, padding: "8px 12px", textAlign: "center" }}>
                ...
                <div style={{ fontSize: 11, color: "#8b5cf6" }}>ت/ماه</div>
              </div>
            </div>
            {(book.gradeLevel || book.academicStage) && (
              <div style={{ marginBottom: 12, display: "flex", gap: 6, flexWrap: "wrap" }}>
                ...
                {book.academicStage && <span style={{ background: "rgba(34,197,94,0.15)", border: "1px solid rgba(34,197,94,0.3)", borderRadius: 999, padding: "2px 8px", fontSize: 11, color: "#4ade80" }}>{book.academicStage}</span>}
              </div>
            )}
            <div style={{ display: "flex", gap: 8 }}>
              <button onClick={() => openEdit(book)} style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", gap: 6, padding: "8px 0", background: "rgba(168,85,247,0.1)", border: "1px solid rgba(168,85,247,0.3)", borderRadius: 8, color: "#a855f7", cursor: "pointer", fontSize: 13, fontFamily: "Vazirmatn" }}><Edit2 size={13} /> ویرایش</button>
      ...
      {showModal && (
        <Modal title={editing ? "ویرایش کتاب" : "افزودن کتاب"} onClose={() => setShowModal(false)}>
          <div style={{ marginBottom: 14 }}>
            <label style={{ display: "block", color: "#c4b5fd", fontSize: 13, marginBottom: 5 }}>عنوان کتاب <span style={{ color: "#f87171" }}>*</span></label>
            <input type="text" value={form.title} onChange={e => setForm({ ...form, title: e.target.value })} style={inputStyle} />
          </div>
          <div style={{ marginBottom: 14 }}>
            <label style={{ display: "block", color: "#c4b5fd", fontSize: 13, marginBottom: 5 }}>مقطع تحصیلی <span style={{ color: "#f87171" }}>*</span></label>
          ...
          <div style={{ marginBottom: 14 }}>
            <label style={{ display: "block", color: "#c4b5fd", fontSize: 13, marginBottom: 5 }}>تعداد درس</label>
            <input type="number" value={form.lessonCount} onChange={e => setForm({ ...form, lessonCount: parseInt(e.target.value) || 0 })} style={inputStyle} />
          </div>
          <div style={{ marginBottom: 14 }}>
            <label style={{ display: "block", color: "#c4b5fd", fontSize: 13, marginBottom: 5 }}>شهریه ماهانه (تومان)</label>
          ...
          <div style={{ marginBottom: 14, display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <label htmlFor="preset" style={{ color: "#c4b5fd", fontSize: 13, cursor: "pointer" }}>کتاب پیش‌فرض سیستم</label>
          </div>
