  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 24 }}>
        <div>
          <h1 style={{ fontSize: 22, fontWeight: 800, color: "#f8f5ff", margin: 0 }}>پکیج‌ها</h1>
          <p style={{ color: "#8b5cf6", fontSize: 14, marginTop: 4 }}>{packages.length} پکیج</p>
        </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(300px, 1fr))", gap: 16 }}>
        {packages.map(pkg => (
          <div key={pkg.id} style={{ background: "rgba(30,18,60,0.85)", border: "1px solid rgba(139,92,246,0.2)", borderRadius: 16, padding: 22, transition: "all 0.3s ease" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 14 }}>
              <h3 style={{ margin: 0, color: "#f8f5ff", fontSize: 16, fontWeight: 700 }}>{pkg.title}</h3>
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 14 }}>
              <div style={{ background: "rgba(245,158,11,0.1)", borderRadius: 10, padding: "10px", textAlign: "center" }}>
                ...
                <div style={{ fontSize: 11, color: "#8b5cf6" }}>قیمت کل (تومان)</div>
              </div>
            ...
            <div style={{ marginBottom: 14 }}>
              <div style={{ fontSize: 12, color: "#8b5cf6", marginBottom: 6 }}>{pkg.bookIds.length} کتاب در پکیج</div>
              <div style={{ display: "flex", flexWrap: "wrap", gap: 4 }}>
                {pkg.bookIds.map(bid => {
                  const book = books.find(b => b.id === bid);
                  return book ? <span key={bid} style={{ background: "rgba(124,58,237,0.15)", border: "1px solid rgba(124,58,237,0.3)", borderRadius: 999, padding: "2px 8px", fontSize: 11, color: "#a855f7" }}>{book.title}</span> : null;
                })}
              </div>
            </div>
            <div style={{ display: "flex", gap: 8 }}>
              <button onClick={() => openEdit(pkg)} style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", gap: 6, padding: "8px 0", background: "rgba(168,85,247,0.1)", border: "1px solid rgba(168,85,247,0.3)", borderRadius: 8, color: "#a855f7", cursor: "pointer", fontSize: 13, fontFamily: "Vazirmatn" }}><Edit2 size={13} /> ویرایش</button>
      ...
      {showModal && (
        <Modal title={editing ? "ویرایش پکیج" : "افزودن پکیج"} onClose={() => setShowModal(false)}>
          <div style={{ marginBottom: 14 }}>
            <label style={{ display: "block", color: "#c4b5fd", fontSize: 13, marginBottom: 5 }}>نام پکیج</label>
            <input value={form.title} onChange={e => setForm({ ...form, title: e.target.value })} style={inputStyle} />
          </div>
          <div style={{ marginBottom: 14 }}>
            <label style={{ display: "block", color: "#c4b5fd", fontSize: 13, marginBottom: 5 }}>تعداد دانش‌آموز مجاز</label>
          ...
          <div style={{ marginBottom: 16 }}>
            <label style={{ display: "block", color: "#c4b5fd", fontSize: 13, marginBottom: 8 }}>کتاب‌های پکیج (کلیک برای انتخاب)</label>
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
              {books.map(b => (
                <button key={b.id} onClick={() => toggleBook(b.id)} style={{
                  ...
                  background: form.bookIds.includes(b.id) ? "rgba(124,58,237,0.3)" : "transparent",
                  border: `1px solid ${form.bookIds.includes(b.id) ? "#7c3aed" : "rgba(139,92,246,0.3)"}`,
                  color: form.bookIds.includes(b.id) ? "#c4b5fd" : "#8b5cf6",
                }}>
                  {b.title}
          ...
          <div style={{ background: "rgba(245,158,11,0.08)", border: "1px solid rgba(245,158,11,0.3)", borderRadius: 12, padding: "14px 16px", marginBottom: 16, display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <div>
              <div style={{ fontSize: 12, color: "#8b5cf6", marginBottom: 2 }}>قیمت کل (محاسبه خودکار)</div>
