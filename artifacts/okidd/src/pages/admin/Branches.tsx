...
export default function AdminBranches() {
  ...
  const addBranchMut = useMutation({ mutationFn: (d: any) => api.post("/branches", { ...d, schoolId: selectedSchoolId }), onSuccess: () => { inv([["branches", String(selectedSchoolId)]]); setAddBranchOpen(false); setBForm({ name: "", address: "" }); } });
  const delBranchMut = useMutation({ mutationFn: (id: number) => api.delete(`/branches/${id}`), onSuccess: () => inv([["branches", String(selectedSchoolId)]]) });
