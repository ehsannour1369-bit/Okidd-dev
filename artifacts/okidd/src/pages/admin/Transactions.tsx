  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 24 }}>
        <div>
          <h1 style={{ fontSize: 22, fontWeight: 800, color: "#78350f", margin: 0 }}>تراکنش‌ها</h1>
          <p style={{ color: "#b45309", fontSize: 14, marginTop: 4 }}>مجموع: {totalRevenue.toLocaleString("fa-IR")} تومان</p>
        </div>
        <div style={{ display: "flex", gap: 10 }}>
          <button onClick={() => exportToExcel(txs)} style={{ display: "flex", alignItems: "center", gap: 8, padding: "10px 16px", background: "rgba(34,197,94,0.15)", border: "1px solid rgba(34,197,94,0.3)", borderRadius: 10, color: "#15803d", fontSize: 14, fontFamily: "Vazirmatn, sans-serif", cursor: "pointer" }}>
      ...
      <div style={{ background: "rgba(255,255,255,0.82)", border: "1px solid rgba(139,92,246,0.2)", borderRadius: 16, overflow: "hidden" }}>
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          ...
          <tbody>
            {txs.map(tx => (
              <tr key={tx.id}>
                <td style={{ padding: "10px 14px", color: "#78350f", fontSize: 14, borderBottom: "1px solid rgba(139,92,246,0.08)" }}>{tx.schoolName ?? tx.schoolId}</td>
                <td style={{ padding: "10px 14px", color: "#92400e", fontSize: 13, borderBottom: "1px solid rgba(139,92,246,0.08)" }}>{tx.packageName ?? tx.packageId}</td>
                <td style={{ padding: "10px 14px", color: "#15803d", fontWeight: 600, fontSize: 14, borderBottom: "1px solid rgba(139,92,246,0.08)" }}>{Number(tx.amount).toLocaleString("fa-IR")}</td>
                ...
                <td style={{ padding: "10px 14px", color: "#92400e", fontSize: 13, borderBottom: "1px solid rgba(139,92,246,0.08)" }}>{tx.paymentMethod === "cash" ? "نقدی" : tx.paymentMethod === "transfer" ? "انتقال" : tx.paymentMethod}</td>
                <td style={{ padding: "10px 14px", borderBottom: "1px solid rgba(139,92,246,0.08)" }}>
                  <span style={{ background: tx.status === "paid" ? "rgba(34,197,94,0.15)" : "rgba(251,191,36,0.15)", color: tx.status === "paid" ? "#15803d" : "#fbbf24", border: `1px solid ${tx.status === "paid" ? "rgba(34,197,94,0.3)" : "rgba(251,191,36,0.3)"}`, borderRadius: 999, padding: "2px 10px", fontSize: 12, fontWeight: 600 }}>
                    {tx.status === "paid" ? "پرداخت شده" : "در انتظار"}
                  </span>
                </td>
                <td style={{ padding: "10px 14px", borderBottom: "1px solid rgba(139,92,246,0.08)" }}>
                  <div style={{ display: "flex", gap: 6 }}>
                    <button onClick={() => openEdit(tx)} style={{ background: "rgba(168,85,247,0.15)", border: "1px solid rgba(168,85,247,0.3)", borderRadius: 6, color: "#d97706", padding: "5px 8px", cursor: "pointer" }}><Edit2 size={13} /></button>
      ...
      {showModal && (
        <Modal title={editing ? "ویرایش تراکنش" : "ثبت تراکنش"} onClose={() => setShowModal(false)}>
          <div style={{ marginBottom: 14 }}>
            <label style={{ display: "block", color: "#92400e", fontSize: 13, marginBottom: 5 }}>مدرسه</label>
          ...
          <div style={{ marginBottom: 14 }}>
            <label style={{ display: "block", color: "#92400e", fontSize: 13, marginBottom: 5 }}>پکیج</label>
          ...
          {[{ label: "مبلغ (تومان)", key: "amount" }, { label: "تخفیف", key: "discount" }, { label: "مانده", key: "balance" }].map(f => (
            <div key={f.key} style={{ marginBottom: 14 }}><label style={{ display: "block", color: "#92400e", fontSize: 13, marginBottom: 5 }}>{f.label}</label><input type="number" value={(form as any)[f.key]} onChange={e => setForm({ ...form, [f.key]: parseFloat(e.target.value) || 0 })} style={inputStyle} /></div>
          ))}
          <div style={{ marginBottom: 14 }}>
            <label style={{ display: "block", color: "#92400e", fontSize: 13, marginBottom: 5 }}>روش پرداخت</label>
          ...
          <div style={{ marginBottom: 14 }}>
            <label style={{ display: "block", color: "#92400e", fontSize: 13, marginBottom: 5 }}>وضعیت</label>
          ...
          <div style={{ marginBottom: 14 }}><label style={{ display: "block", color: "#92400e", fontSize: 13, marginBottom: 5 }}>یادداشت</label><textarea value={form.notes} onChange={e => setForm({ ...form, notes: e.target.value })} rows={2} style={{ ...inputStyle, resize: "vertical" }} /></div>
