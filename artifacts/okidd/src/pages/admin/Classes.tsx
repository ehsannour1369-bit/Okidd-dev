    ...
    <div>
      ...
      <div style={{ marginBottom: 20, padding: "16px", background: "rgba(255,255,255,0.82)", border: "1px solid rgba(139,92,246,0.2)", borderRadius: 14, display: "flex", alignItems: "center", gap: 12 }}>
        ...
        <select
          ...
          style={{ width: "100%", maxWidth: 400, background: "rgba(255,252,235,0.90)", border: "1px solid rgba(139,92,246,0.3)", borderRadius: 10, color: "#78350f", padding: "10px 12px", fontSize: 14, fontFamily: "Vazirmatn, sans-serif", outline: "none", cursor: "pointer" }}
        >
      ...
      {selectedSchoolId && (
        <>
          <h1 style={{ fontSize: 22, fontWeight: 800, color: "#78350f", marginBottom: 24 }}>کلاس‌ها</h1>
          {branches.map(branch => {
            const branchGLs = gradeLevels.filter(gl => gl.branchId === branch.id);
            return (
              <div key={branch.id} style={{ marginBottom: 28 }}>
                <h2 style={{ color: "#d97706", fontSize: 16, fontWeight: 700, marginBottom: 12, display: "flex", alignItems: "center", gap: 8 }}>
                ...
                {branchGLs.map(gl => {
                  ...
                  return (
                    <div key={gl.id} style={{ marginBottom: 16, paddingRight: 16 }}>
                      <h3 style={{ color: "#92400e", fontSize: 14, fontWeight: 600, marginBottom: 10 }}>{gl.name}</h3>
                      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: 12 }}>
                          ...
                          return gradeClasses.map(cls => (
                            <Link key={cls.id} href={`/admin/classes/${cls.id}`} style={{ textDecoration: "none" }}>
                                <div style={{
                                  ...
                                  onMouseOver={e => { const el = e.currentTarget as HTMLElement; el.style.borderColor = "#7c3aed"; el.style.boxShadow = "0 0 20px rgba(180,83,9,0.12)"; el.style.transform = "translateY(-2px)"; }}
                                  onMouseOut={e => { const el = e.currentTarget as HTMLElement; el.style.borderColor = "rgba(180,83,9,0.15)"; el.style.boxShadow = "none"; el.style.transform = "translateY(0)"; }}
                                >
                                  <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
                                    ...
                                    <div>
                                      <div style={{ fontWeight: 700, color: "#78350f", fontSize: 14 }}>{cls.name}</div>
                                      <div style={{ fontSize: 11, color: "#b45309" }}>{grade.name}</div>
                                    </div>
                                  </div>
                        ...
                        {glGrades.length === 0 && <p style={{ color: "#b45309", fontSize: 13 }}>کلاسی وجود ندارد</p>}
                      </div>
                    </div>
                  );
                })}
