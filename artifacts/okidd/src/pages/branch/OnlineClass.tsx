...
const IS: React.CSSProperties = {
  width: "100%", background: "rgba(240,253,250,0.9)", border: "1px solid rgba(13,148,136,0.30)",
...
const DAY_COLORS = ["#0d9488","#10b981","#06b6d4","#2dd4bf","#14b8a6","#059669"];
...
function SessionStatusBadge({ status }: { status: string }) {
  ...
  return (
    <span style={{
      ...
      <span style={{ width: 7, height: 7, borderRadius: "50%", background: active ? "#10b981" : "#9ca3af", display: "inline-block" }} />
      {active ? "فعال" : "پایان‌یافته"}
    </span>
  );
}

export default function BranchOnlineClass() {
  ...
  const { data: classes = [] } = useQuery<any[]>({
    queryKey: ["classes-for-online-branch", (user as any)?.branchId],
    queryFn: () => api.get(`/classes?branchId=${(user as any)?.branchId ?? 0}`),
    enabled: !!user,
  });
  ...
  const { data: classTeachers = [] } = useQuery<any[]>({
    queryKey: ["class-teachers-for-schedule-branch", selectedClassId],
    ...
    <div dir="rtl" style={{ fontFamily: "Vazirmatn, sans-serif", paddingBottom: 32 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 24 }}>
        ...
        <div>
          ...
          <p style={{ margin: 0, fontSize: 13, color: C.text2 }}>مدیریت برنامه هفتگی و جلسات آنلاین شعبه</p>
        </div>
      </div>
