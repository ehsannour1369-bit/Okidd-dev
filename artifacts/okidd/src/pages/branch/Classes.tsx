...
export default function BranchClasses() {
  ...
  const branchClasses = classes.filter((c: any) => branchGradeIds.has(c.gradeId)).map((c: any) => ({
    ...c,
  ...
  return (
    <div>
      <div style={{ marginBottom: 24 }}>
        ...
        <p style={{ color: "#8b5cf6", fontSize: 14, marginTop: 4 }}>مدیریت کلاس‌ها، معلمان، دانش‌آموزان و کتاب‌ها</p>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(260px, 1fr))", gap: 14 }}>
        {branchClasses.map((cls: any) => (
          <div key={cls.id} style={{ background: "rgba(30,18,60,0.85)", border: "1px solid rgba(139,92,246,0.2)", borderRadius: 14, padding: 18, transition: "all 0.2s ease", cursor: "pointer" }}
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
              ...
              <div>
                ...
                <div style={{ fontSize: 11, color: "#8b5cf6" }}>{grades.find((g: any) => g.id === cls.gradeId)?.name ?? ""}</div>
              </div>
            </div>
