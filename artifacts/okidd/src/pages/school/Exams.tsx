...
const inputStyle = { width: "100%", background: "rgba(245,243,255,0.90)", border: "1px solid rgba(139,92,246,0.3)", borderRadius: 10, color: "#1e1b4b", padding: "10px 12px", fontSize: 14, fontFamily: "Vazirmatn, sans-serif", outline: "none", direction: "rtl" as const };
    ...
    <div>
      ...
      {showForm && (
        <div style={{ background: "rgba(255,255,255,0.82)", border: "1px solid rgba(124,58,237,0.3)", borderRadius: 16, padding: 20, marginBottom: 20 }}>
          <h3 style={{ color: "#1e1b4b", marginTop: 0, marginBottom: 16, fontSize: 16, fontWeight: 700 }}>افزودن امتحان</h3>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <div style={{ gridColumn: "1/-1" }}><label style={{ display: "block", color: "#3730a3", fontSize: 13, marginBottom: 5 }}>نام درس</label><input value={form.lessonName} onChange={e => setForm({ ...form, lessonName: e.target.value })} style={inputStyle} /></div>
            <div><label style={{ display: "block", color: "#3730a3", fontSize: 13, marginBottom: 5 }}>تاریخ امتحان</label><input value={form.examDate} onChange={e => setForm({ ...form, examDate: e.target.value })} style={inputStyle} placeholder="1403/09/15" /></div>
            <div><label style={{ display: "block", color: "#3730a3", fontSize: 13, marginBottom: 5 }}>ساعت</label><input value={form.examTime} onChange={e => setForm({ ...form, examTime: e.target.value })} style={inputStyle} placeholder="08:00" /></div>
            <div style={{ gridColumn: "1/-1" }}><label style={{ display: "block", color: "#3730a3", fontSize: 13, marginBottom: 5 }}>صفحات</label><input value={form.examPages} onChange={e => setForm({ ...form, examPages: e.target.value })} style={inputStyle} placeholder="صفحه ۱ تا ۲۵" /></div>
          </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        {exams.map(exam => (
          <div key={exam.id} style={{ background: "rgba(255,255,255,0.82)", border: "1px solid rgba(139,92,246,0.2)", borderRadius: 14, padding: "16px 20px", display: "flex", alignItems: "center", gap: 16 }}>
        ...
        {exams.length === 0 && <p style={{ color: "#4f46e5", textAlign: "center", padding: 30 }}>هیچ امتحانی ثبت نشده</p>}
      </div>
    </div>
  );
}
