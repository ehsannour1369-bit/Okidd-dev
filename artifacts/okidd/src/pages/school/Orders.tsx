...
const ACCENT   = "#6366f1";
const ACCENT_BG = "#f0f0ff";
const ACCENT_BORDER = "#ede9fe";
...
function printInvoice(order: Order) {
  ...
  .header{display:flex;justify-content:space-between;margin-bottom:32px;padding-bottom:20px;border-bottom:2px solid #6d28d9}
  .logo{font-size:26px;font-weight:800;color:#6d28d9}.logo span{display:block;font-size:13px;font-weight:400;color:#6b7280;margin-top:4px}
  ...
  table{width:100%;border-collapse:collapse;margin:20px 0}thead tr{background:#f3e8ff}th{padding:10px;text-align:right;color:#6d28d9;font-size:13px}
  td{font-size:13px;color:#374151}.totals{margin-top:16px;border:1px solid #ede9fe;border-radius:12px;padding:16px;background:#faf5ff}
  ...
  ${order.notes ? `<div style="margin-top:14px;background:#ede9fe;border-radius:8px;padding:10px 14px;font-size:13px;color:#3730a3"><b>یادداشت:</b> ${order.notes}</div>` : ""}
  <div class="footer">این فاکتور توسط سیستم اوکید صادر شده است</div>
  <div style="text-align:center;margin-top:24px"><button onclick="window.print()" style="background:#6d28d9;color:#fff;border:none;border-radius:8px;padding:10px 28px;font-size:14px;font-family:inherit;cursor:pointer;font-weight:700">چاپ فاکتور</button></div>
  </body></html>`;
      ...
      <div style={{ maxWidth: 860, margin: "0 auto", padding: "20px 16px" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 24, flexWrap: "wrap" }}>
          ...
          <button onClick={() => setShowForm(true)}
            style={{ display: "flex", alignItems: "center", gap: 6, background: `linear-gradient(135deg,${ACCENT},#818cf8)`, color: "#fff", border: "none", borderRadius: 10, padding: "9px 18px", cursor: "pointer", fontFamily: "inherit", fontSize: 14, fontWeight: 600 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          {orders.map(order => (
            <div key={order.id} style={{ background: "#fff", borderRadius: 16, border: `1.5px solid ${ACCENT_BORDER}`, overflow: "hidden", boxShadow: "0 2px 8px rgba(99,102,241,0.06)" }}>
              <div style={{ padding: "14px 16px", display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
                ...
                <div style={{ display: "flex", gap: 8, flexShrink: 0 }}>
                  ...
                  <button onClick={() => reorderMut.mutate(order)}
                    style={{ display: "flex", alignItems: "center", gap: 4, background: ACCENT_BG, color: ACCENT, border: `1.5px solid #c7d2fe`, borderRadius: 8, padding: "5px 10px", cursor: "pointer", fontSize: 12, fontFamily: "inherit", fontWeight: 600 }}>
                    <RefreshCw size={12} /> تکرار
                  </button>
                  <button onClick={() => setExpanded(expanded === order.id ? null : order.id)}
                    style={{ background: "#f5f3ff", border: `1.5px solid ${ACCENT_BORDER}`, borderRadius: 8, padding: "5px 10px", cursor: "pointer", color: ACCENT }}>
              ...
              {expanded === order.id && (
                <div style={{ borderTop: `1px solid ${ACCENT_BORDER}`, background: "#faf5ff", padding: "14px 16px", display: "flex", flexDirection: "column", gap: 10 }}>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 3, fontSize: 13, alignItems: "flex-end" }}>
                    ...
                    <div style={{ display: "flex", gap: 16, borderTop: `1px solid ${ACCENT_BORDER}`, paddingTop: 6, marginTop: 2 }}>
                  ...
                  {order.notes && <div style={{ fontSize: 12, color: ACCENT, background: ACCENT_BORDER, padding: "6px 10px", borderRadius: 6 }}>یادداشت: {order.notes}</div>}
                  {/* Receipt viewer */}
                  {order.receiptUrl && (
                    <div style={{ background: "#fff", borderRadius: 10, border: `1.5px solid ${ACCENT_BORDER}`, padding: "10px 12px", display: "flex", alignItems: "center", gap: 10 }}>
                      <div style={{ width: 44, height: 44, borderRadius: 8, overflow: "hidden", border: `1.5px solid ${ACCENT_BORDER}`, background: "#f5f3ff", flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center" }}>
                  ...
                  {order.status === "paid" && (
                    <div style={{ textAlign: "center" }}>
                      <button onClick={() => printInvoice(order)}
                        style={{ display: "inline-flex", alignItems: "center", gap: 6, background: `linear-gradient(135deg,${ACCENT},#818cf8)`, color: "#fff", border: "none", borderRadius: 9, padding: "9px 22px", cursor: "pointer", fontFamily: "inherit", fontWeight: 600, fontSize: 13 }}>
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
      <div style={{ background: "#fff", borderRadius: 18, padding: 22, maxWidth: 560, width: "100%", maxHeight: "92vh", overflowY: "auto", fontFamily: "Vazirmatn, sans-serif", direction: "rtl" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 18 }}>
          <h2 style={{ margin: 0, fontSize: 17, color: "#1e1b4b" }}>ثبت سفارش جدید</h2>
        ...
        <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
          ...
          <button onClick={submit} disabled={loading}
            style={{ padding: "9px 20px", background: `linear-gradient(135deg,${accent},#818cf8)`, color: "#fff", border: "none", borderRadius: 10, cursor: "pointer", fontFamily: "inherit", fontWeight: 600 }}>
