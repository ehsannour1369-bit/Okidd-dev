...
export default function TeacherDashboard() {
  ...
  const ACCORDION_CARDS = [
    { id: "schools", label: "مدارس من", emoji: "🏫", desc: `${schools.length} مدرسه`, open: schoolsOpen, color: "#f59e0b", dark: "#d97706" },
  ...
  const classPill = (cls: any, selected: any, onClick: () => void) => (
    <button key={cls.id} onClick={onClick} style={{ padding: "6px 14px", borderRadius: 999, fontSize: 13, cursor: "pointer", fontFamily: "Vazirmatn, sans-serif", background: selected?.id === cls.id ? `${P}22` : "rgba(245,158,11,0.08)", border: `1px solid ${selected?.id === cls.id ? P : `${P}35`}`, color: selected?.id === cls.id ? PD : TEXT2 }}>
      {cls.name}
    </button>
  );
      ...
      <div style={{ position: "relative", zIndex: 1, maxWidth: 860, margin: "0 auto" }}>
        ...
        <div style={{ ...cardAnim(0), marginBottom: 28 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
            ...
            <div>
              <h1 style={{ fontSize: 22, fontWeight: 900, color: TEXT, margin: 0 }}>داشبورد معلم</h1>
        ...
        {ACCORDION_CARDS.map((ac, idx) => (
          <div key={ac.id} style={{ marginBottom: 14, ...cardAnim(idx + 1) }}>
            <div
              ...
              style={{ ...glassCard(ac.color, { padding: "20px 22px", display: "flex", alignItems: "center", gap: 16, cursor: "pointer", borderRadius: ac.open ? "22px 22px 0 0" : 22, borderBottomColor: ac.open ? "transparent" : "rgba(255,255,255,0.92)", boxShadow: ac.open ? `0 0 24px ${ac.color}30, 0 2px 8px rgba(0,0,0,0.06)` : `0 8px 32px ${ac.color}20, 0 2px 8px rgba(0,0,0,0.06)` }) }}
            ...
            >
              ...
              <div style={{ flex: 1 }}>
                <div style={{ fontWeight: 800, fontSize: 16, color: TEXT, marginBottom: 3 }}>{ac.label}</div>
                <div style={{ color: ac.dark, fontSize: 13, fontWeight: 500 }}>{ac.desc}</div>
              </div>
              <div style={{ width: 30, height: 30, borderRadius: 10, background: `${ac.color}20`, border: `1px solid ${ac.color}40`, display: "flex", alignItems: "center", justifyContent: "center" }}>
                {ac.open ? <ChevronDown size={17} color={ac.dark} /> : <ChevronLeft size={17} color={`${ac.color}88`} />}
              </div>
              <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, height: 4, background: `linear-gradient(90deg, ${ac.color}, ${ac.color}40)`, borderRadius: ac.open ? 0 : "0 0 22px 22px" }} />
            </div>

            {ac.open && (
              <div style={{ background: "rgba(255,255,255,0.60)", backdropFilter: "blur(18px)", border: "1.5px solid rgba(255,255,255,0.92)", borderTop: "none", borderRadius: "0 0 22px 22px", padding: 20 }}>

                {ac.id === "schools" && (
                  schools.length === 0 ? <div style={{ color: TEXT2, textAlign: "center", padding: 20 }}>هیچ مدرسه‌ای یافت نشد</div> : (
                    <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                      {schools.map((school: any) => (
                        <div key={school.id}>
                          <button onClick={() => setSelectedSchool(selectedSchool?.id === school.id ? null : school)}
                            style={{ width: "100%", display: "flex", alignItems: "center", justifyContent: "space-between", padding: "11px 16px", background: selectedSchool?.id === school.id ? `${P}18` : "rgba(245,158,11,0.06)", border: `1px solid ${selectedSchool?.id === school.id ? `${P}50` : `${P}22`}`, borderRadius: 12, cursor: "pointer", color: TEXT, fontFamily: "Vazirmatn", fontWeight: 600, fontSize: 14 }}>
                            <span><School size={15} style={{ display: "inline", verticalAlign: "middle", marginLeft: 6, color: PD }} />{school.name}</span>
                            <span style={{ fontSize: 12, color: TEXT2 }}>{school.classes?.length ?? 0} کلاس</span>
                          </button>
                          {selectedSchool?.id === school.id && (
                            <div style={{ marginTop: 7, paddingRight: 14 }}>
                              {school.classes?.length === 0 ? <div style={{ color: TEXT2, padding: "8px 0", fontSize: 13 }}>کلاسی یافت نشد</div> : (
                                <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                                  {school.classes.map((cls: any) => (
                                    <div key={cls.id}>
                                      <button onClick={() => setSelectedClass(selectedClass?.id === cls.id ? null : cls)}
                                        style={{ width: "100%", display: "flex", alignItems: "center", justifyContent: "space-between", padding: "9px 14px", background: selectedClass?.id === cls.id ? "rgba(59,130,246,0.12)" : "rgba(245,158,11,0.05)", border: `1px solid ${selectedClass?.id === cls.id ? "rgba(59,130,246,0.4)" : `${P}18`}`, borderRadius: 10, cursor: "pointer", color: TEXT, fontFamily: "Vazirmatn", fontSize: 13 }}>
                                        <span>📚 {cls.name}</span>
                                        <ChevronDown size={13} color={TEXT2} />
                                      </button>
                                      {selectedClass?.id === cls.id && (
                                        <div style={{ marginTop: 5, background: "rgba(255,255,255,0.55)", borderRadius: 10, padding: 12, border: "1px solid rgba(59,130,246,0.15)" }}>
                                          <div style={{ fontSize: 13, color: "#2563eb", marginBottom: 10, fontWeight: 700, display: "flex", alignItems: "center", gap: 6 }}>
                                            <Users size={13} /> دانش‌آموزان کلاس
                                          </div>
                                          {classStudents.length === 0 ? <div style={{ color: TEXT2, fontSize: 13 }}>دانش‌آموزی یافت نشد</div> : (
                                              ...
                                              {classStudents.map((s: any) => (
                                                <div key={s.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "7px 12px", background: `${P}0a`, borderRadius: 8 }}>
                                                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                                                    ...
                                                    <span style={{ color: TEXT, fontSize: 13, fontWeight: 600 }}>{s.name}</span>
                                                  </div>
                                                  <div style={{ color: TEXT2, fontSize: 11 }}>آخرین ورود: {fmtDate(s.lastLoginAt)}</div>
                                                </div>
                                              ))}
                                            </div>
                                          )}
                ...
                {ac.id === "progress" && (
                  <div>
                    <div style={{ marginBottom: 14 }}>
                      <label style={{ display: "block", color: TEXT, fontSize: 13, marginBottom: 8, fontWeight: 700 }}>انتخاب کلاس:</label>
                    ...
                    {progressClass && (
                      progressBooks.length === 0
                        ? <div style={{ color: TEXT2, textAlign: "center", padding: 20, fontSize: 13 }}>هیچ کتابی برای این کلاس تعریف نشده</div>
                        : progressBooks.map((book: any) => <LessonUnlockBook key={book.id} book={book} classId={progressClass.id} unlocks={lessonUnlocks} dates={progressChartDates} accentColor={P} accentDark={PD} />)
                    )}
                  </div>
                )}
                ...
                {ac.id === "perf" && (
                  <div>
                    ...
                    {perfClass && (
                      perfData.length === 0
                        ? <div style={{ color: TEXT2, textAlign: "center", padding: 20, fontSize: 13 }}>دانش‌آموزی در این کلاس وجود ندارد</div>
                        : (
                              ...
                              <thead>
                                <tr>
                                  {["دانش‌آموز", "آخرین حضور", "زمان در برنامه", "امتیاز کل", "پیشرفت کتاب‌ها"].map(h => (
                                    <th key={h} style={{ textAlign: "right", padding: "10px 12px", color: TEXT2, fontSize: 12, fontWeight: 700, background: `${P}10`, borderBottom: `1px solid ${P}22` }}>{h}</th>
                                  ))}
                                </tr>
                              </thead>
                              <tbody>
                                {perfData.map((s: any) => (
                                  <tr key={s.id}>
                                    <td style={{ padding: "10px 12px", borderBottom: `1px solid ${P}14` }}>
                                      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                                        <span style={{ fontSize: 17 }}>{s.gender === "female" ? "👧" : "👦"}</span>
                                        <div>
                                          <div style={{ color: TEXT, fontWeight: 700, fontSize: 13 }}>{s.name}</div>
                                          <div style={{ color: TEXT2, fontSize: 11, direction: "ltr" }}>{s.email}</div>
                                        </div>
                                      </div>
                                    </td>
                                    <td style={{ padding: "10px 12px", borderBottom: `1px solid ${P}14` }}>
                                      <div style={{ display: "flex", alignItems: "center", gap: 4, color: TEXT2, fontSize: 12 }}>
                                        <Clock size={12} color="#2563eb" />{fmtDate(s.lastPresenceAt)}
                                      </div>
                                    </td>
                                    <td style={{ padding: "10px 12px", borderBottom: `1px solid ${P}14`, color: "#16a34a", fontSize: 13, fontWeight: 600 }}>{fmtDuration(s.totalMinutesInApp ?? 0)}</td>
                                    <td style={{ padding: "10px 12px", borderBottom: `1px solid ${P}14` }}>
                                      <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
                                        <Star size={13} color="#d97706" />
                                        <span style={{ color: "#d97706", fontWeight: 800 }}>{(s.totalScore ?? 0).toLocaleString("fa-IR")}</span>
                                      </div>
                                    </td>
                                    <td style={{ padding: "10px 12px", borderBottom: `1px solid ${P}14` }}>
                                          ...
                                          return (
                                            <div key={bp.bookId}>
                                              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 2 }}>
                                                <span style={{ fontSize: 11, color: TEXT }}>{bp.bookTitle}</span>
                                                <span style={{ fontSize: 11, color: TEXT2 }}>{bp.completedLessons}/{bp.lessonCount}</span>
                                              </div>
                                              <div style={{ height: 4, background: `${P}20`, borderRadius: 999, overflow: "hidden", width: 100 }}>
                                                <div style={{ height: "100%", width: `${pct}%`, background: `linear-gradient(90deg, ${P}, ${S})`, borderRadius: 999 }} />
                                              </div>
                                            </div>
                                          );
                                        })}
                                        {!s.bookProgress?.length && <span style={{ color: TEXT2, fontSize: 12 }}>بدون پیشرفت</span>}
                                      </div>
                                    </td>
                                  </tr>
                                ))}
...
function LessonUnlockBook({ book, classId, unlocks, dates, accentColor, accentDark }: { book: any; classId: number; unlocks: any[]; dates: any[]; accentColor: string; accentDark: string }) {
  ...
  return (
    <div style={{ background: `${accentColor}0c`, borderRadius: 12, padding: 14, marginBottom: 10, border: `1px solid ${accentColor}25` }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
        <BookOpen size={15} color={accentDark} />
        <span style={{ color: TEXT, fontWeight: 700, fontSize: 14 }}>{book.title}</span>
        <span style={{ color: TEXT2, fontSize: 12 }}>({book.lessonCount} درس)</span>
      </div>
      <div style={{ display: "flex", flexWrap: "wrap", gap: 7 }}>
        {lessons.map(lessonId => {
          ...
          return (
            <button key={lessonId} onClick={() => mutate({ lessonId, unlock: !isUnlocked })} style={{ minWidth: 70, height: 54, borderRadius: 10, cursor: "pointer", fontFamily: "Vazirmatn", background: isUnlocked ? "rgba(34,197,94,0.12)" : "rgba(239,68,68,0.08)", border: `1px solid ${isUnlocked ? "rgba(34,197,94,0.40)" : "rgba(239,68,68,0.28)"}`, color: isUnlocked ? "#16a34a" : "#dc2626", display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", gap: 3, padding: "6px 10px" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 3, fontSize: 12, fontWeight: 700 }}>
                {isUnlocked ? <Unlock size={10} /> : <Lock size={10} />} {lessonId}
              </div>
              {date ? <span style={{ fontSize: 10, color: "#2563eb", direction: "ltr", fontWeight: 600 }}>{date}</span>
