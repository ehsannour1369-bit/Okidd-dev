...
router.get("/students/search-by-national-id", async (req, res) => {
  ...
  if (!nationalId || nationalId.length < 3) return res.json([]);
...
router.post("/parent-students", async (req, res) => {
  ...
  if (!parentId || !studentId) return res.status(400).json({ error: "parentId و studentId الزامی است" });
  ...
  if (!parent) return res.status(400).json({ error: "کاربر مورد نظر نقش والد ندارد" });

  // Verify student
  const [student] = await db.select().from(usersTable).where(and(eq(usersTable.id, studentId), eq(usersTable.role, "student")));
  if (!student) return res.status(400).json({ error: "دانش‌آموز یافت نشد" });

  // Check max 2 active parents per student
  ...
  if (existingParents.length >= 2) {
    return res.status(400).json({ error: "هر دانش‌آموز حداکثر ۲ والد می‌تواند داشته باشد" });
  }

  // Check duplicate
  const duplicate = existingParents.find(p => p.parentId === parentId);
  if (duplicate) return res.status(400).json({ error: "این اتصال قبلاً برقرار شده است" });
...
// PATCH /parent-students/:id — update relation type or notes
router.patch("/parent-students/:id", async (req, res) => {
  ...
  if (!updated) return res.status(404).json({ error: "اتصال یافت نشد" });
  res.json(updated);
});

// DELETE /parent-students/:id — remove link (soft delete)
